...
export default {
    ...
    methods: {
        toggleStatus (item, status) {
            RestApi.putData(baseUrl, `/customers/status/${item.id}/`, { ...item, status: status }).then(response => {
        ...
        loadData () {
            const params = Object.assign({}, this.search, { page: this.pagination.currentPage, per_page: this.pagination.perPage, status: 0 })
            this.$store.dispatch('mutedLoad', { loading: true})
            RestApi.getData(baseUrl, 'customers', params).then(response => {
                if (response.success) {

                    ...
                    <div class="overflow-auto">
                        <b-table thead-class="thead-color" emptyText="Data Not Found" small show-empty bordered hover :items="itemList" :fields="fields">
                            ...
                            <template v-slot:cell(image)="data">
                                <img v-if="data.item.book_image" :src="'/'+data.item.book_image" alt="Book Image" width="100" class="img-thumbnail" />
                            </template>
                            ...
                            <template v-slot:cell(action)="data">
                                ...
                                <b-button title="Active/Inactive" class="ml-2 btn btn-primary btn-sm" @click="editRouter(data.item, 'books/create')"> <i class="ri-edit-line"></i> </b-button>
                            </template>
                        </b-table>
                    </div>
                </b-overlay>
...
export default {
    ...
    computed: {
        fields () {
            const labels = [
                ...
                { label: 'Series', class: 'text-center' },
    ...
    methods: {
        toggleStatus (item, status) {
            RestApi.putData(baseUrl, `/status/books/${item.id}/`, { ...item, status: status }).then(response => {
        ...
        loadData () {
            ...
            RestApi.getData(baseUrl, 'books', params).then(response => {
                if (response.success) {
